    ...
    public partial class NUPLunasBayarFin : System.Web.UI.Page
	{
		...
		private void Fill()
		{
            ...
            if (dtNUP.Rows.Count > 0)
            {
                ...
                string notts = Db.SingleString("SELECT NoTTS FROM MS_NUP_PELUNASAN WHERE NoNUP='" + NoNUP + "' AND Tipe = '" + Tipe + "' AND Project = '" + Project + "' AND PelunasanKe=1");
                string notts2 = Db.SingleString("SELECT NoTTS FROM MS_NUP_PELUNASAN WHERE NoNUP='" + NoNUP + "' AND Tipe = '" + Tipe + "' AND Project = '" + Project + "' AND PelunasanKe=2");

                customer.Text = namaCS;
                agent.Text = namaAG;
                
                asp.HRef = "javascript:openPopUp('/nup/PrintTTS.aspx?NoTTS=" + notts + "&project=" + Project + "','920','650')";
                asp2.HRef = "javascript:openPopUp('/nup/PrintTTS.aspx?NoTTS=" + notts2 + "&project=" + Project + "','920','650')";
            }
