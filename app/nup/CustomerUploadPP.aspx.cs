...
{
    public partial class CustomerUploadPP : System.Web.UI.Page
    {
        int Berhasil = 0;
        int Gagal = 0;
        string BarisGagal = "";
        ...
        private void Bind()
        {
            ...
            DataTable rs = Db.Rs("SELECT Nama FROM MS_AGENT WHERE Project = '" + project.SelectedValue + "' ORDER BY NoAgent");
            ...
            DataTable rs2 = Db.Rs("SELECT Nama FROM REF_JENISPROPERTI WHERE Project = '" + project.SelectedValue + "'");
            for (int i = 0; i < rs2.Rows.Count; i++)
            {
                if (y.Length != 0) y.Append("/");
                y.Append(rs2.Rows[i][0].ToString());
            }

            TableCell d = rule.Rows[27].Cells[4];
            d.Text = y.ToString();

            System.Text.StringBuilder z = new System.Text.StringBuilder();

            DataTable rs3 = Db.Rs("SELECT Rekening FROM "+Mi.DbPrefix+"FINANCEAR..REF_ACC WHERE Project = '" + project.SelectedValue + "'");
            for (int k = 0; k < rs3.Rows.Count; k++)
            {
                if (z.Length != 0) z.Append("/");
                z.Append(rs3.Rows[k][0].ToString());
            }

            TableCell gg = rule.Rows[24].Cells[4];
            gg.Text = z.ToString();
        }
        ...
        private bool Save(DataTable rs, int i)
        {
            ...
            int AdaNUP = Db.SingleInteger("Select Count(*) From MS_NUP WHERE NoNup = " + NoNUP + " AND TIPE = '" + JenisProperti + "' AND Project = '" + project.SelectedValue + "'");
            ...
            if (AdaNUP == 0)
            {
                ...
                Db.Execute("UPDATE MS_NUP SET "
                    ...
                    + " WHERE NoNUP='" + NoNUP + "' AND Tipe = '" + JenisProperti + "' AND Project = '" + project.SelectedValue + "'");
                ...
                DataTable rsLog = Db.Rs("SELECT "
                    ...
                    + " WHERE MS_NUP.NoNUP = '" + NoNUP + "' AND MS_NUP.Tipe = '" + JenisProperti + "' AND MS_NUP.Project = '" + project.SelectedValue + "'");
                ...
                string NoTTS = "";
                string Acc = Db.SingleString("SELECT Acc FROM " + Mi.DbPrefix + "FINANCEAR..REF_ACC WHERE Rekening = '" + rekpers + "'");
                ...
                //kode no tts
                NoTTS = SaveTTS(NoNUP, cabar, Acc, ketbayar, TglTransferNUP, Db.SingleInteger("SELECT NoCustomer FROM MS_NUP WHERE NoNUP='" + NoNUP + "' AND Tipe = '" + JenisProperti + "' AND Project = '" + project.SelectedValue + "'"), NilaiNUP, JenisProperti, project.SelectedValue);
                ...
                Db.Execute("UPDATE MS_NUP SET NilaiBayar = '" + valuenup + "' WHERE NoNUP='" + NoNUP + "' AND Tipe = '" + JenisProperti + "' AND Project = '" + project.SelectedValue + "'");
                ...
                rsLog = Db.Rs("SELECT "
                    ...
                    + " WHERE A.NoNUP = '" + NoNUP + "' AND A.Tipe = '" + JenisProperti + "' AND A.Project = '" + project.SelectedValue + "'");
        ...
        private string SaveTTS(string NoNUP, string cabar, string rekpers, string ketbayar, DateTime TglTransfer, int NoCustomer, decimal nilai, string Jenis, string Projectt)
        {
            int roman = Convert.ToInt32(Cf.Bulan(DateTime.Now));
            ...
            string NoTTS2 = Numerator.TTS(TglTTS.Month, TglTTS.Year, Projectt);
            ...
            Db.Execute("UPDATE " + Mi.DbPrefix + "FINANCEAR..MS_TTS_LOG SET Project = '" + Projectt + "' WHERE LogID  = " + LogID);
