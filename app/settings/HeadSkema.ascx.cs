    ...
    public partial class HeadSkema : System.Web.UI.UserControl
    {
        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                int p = Db.SingleInteger("SELECT ISNULL((SELECT TOP 1 Nomor FROM " + Mi.DbPrefix + "MARKETINGJUAL..REF_SKEMA WHERE Nomor < " + Nomor + " AND Project IN (" + Act.ProjectListSql + ") ORDER BY Nomor DESC),0)");
                int n = Db.SingleInteger("SELECT ISNULL((SELECT TOP 1 Nomor FROM " + Mi.DbPrefix + "MARKETINGJUAL..REF_SKEMA WHERE Nomor > " + Nomor + " AND Project IN (" + Act.ProjectListSql + ") ORDER BY Nomor ASC),0)");
                ...
                string strSql = "SELECT Nomor,Nama FROM " + Mi.DbPrefix + "MARKETINGJUAL..REF_SKEMA WHERE Nomor = " + Nomor;
