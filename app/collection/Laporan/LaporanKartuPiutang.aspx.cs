    ...
    public partial class LaporanKartuPiutang : System.Web.UI.Page
    {
        ...
        private void Fill()
        {
            ...
            for (int i = 0; i < rs.Rows.Count; i++)
            {
                ...
                //TAGIHAN
                l = new Label();
                decimal Tagihan = Db.SingleDecimal("SELECT ISNULL(SUM(NilaiTagihan),0) FROM ISC064_MARKETINGJUAL..MS_TAGIHAN WHERE NoKontrak = '" + Cf.Str(NoKontrak) + "' AND Tipe IN ('BF','DP','ANG')");
                ...
                //BIAYA
                l = new Label();
                decimal Biaya = Db.SingleDecimal("SELECT ISNULL(SUM(NilaiTagihan),0) FROM ISC064_MARKETINGJUAL..MS_TAGIHAN WHERE NoKontrak = '" + Cf.Str(NoKontrak) + "' AND Tipe IN ('ADM')");
