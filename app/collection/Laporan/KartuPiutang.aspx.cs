	...
	public partial class KartuPiutang : System.Web.UI.Page
	{
		...
		{	
			...
			for(int i = 0; i < rs.Rows.Count; i++)
			{
				...
				//HEADER
				l = new Label();
				l.Text = "<tr><td colspan='8'><h1 class='title'>KARTU PIUTANG</h1><h2>" + Mi.Pt + "</h2><h2>Pancoran Riverside</h2></td></tr>";
				report.Controls.Add(l);
				...
				//NAMA
				l = new Label();
				string Nama = Db.SingleString("SELECT Nama FROM ISC064_MARKETINGJUAL..MS_CUSTOMER "
		...
		private void FillTagihan(string NoKontrak)
		{
			string strSql = "SELECT *"
				+ " FROM ISC064_MARKETINGJUAL..MS_TAGIHAN"
				+ " WHERE NoKontrak = '" + Cf.Pk(NoKontrak) + "' AND NamaTagihan NOT LIKE '%BATAL%' AND Tipe<>'ADM'"
				;
			...
			for(int i = 0; i < rs.Rows.Count; i++)
			{
				...
				decimal a = Db.SingleDecimal("SELECT NilaiPelunasan FROM ISC064_MARKETINGJUAL..MS_PELUNASAN WHERE NoTagihan = '" + rs.Rows[i]["NoUrut"]+ "' AND NoKontrak = '" + rs.Rows[i]["NoKontrak"]+ "' ORDER BY TglPelunasan DESC");
				decimal b = Db.SingleDecimal("SELECT ISNULL (SUM(NilaiPelunasan),0) FROM ISC064_MARKETINGJUAL..MS_PELUNASAN WHERE NoKontrak = '" + rs.Rows[i]["NoKontrak"]+ "' ");
				decimal sisa = 0;
				sisa = Total - b;
				if (a == 0)
					x.Append("<td>&nbsp;</td>");
				...
				{
					x.Append("<tr>");
					x.Append("<td colspan=5 style='border-top: solid black 1px;'><strong>TOTAL</strong></td>");
		...
		{
			System.Text.StringBuilder x = new System.Text.StringBuilder();
			string strSql = "SELECT ManualTTS FROM ISC064_MARKETINGJUAL..MS_PELUNASAN " 
				+ " WHERE NoKontrak = '" + Cf.Pk(NoKontrak) + "'"
				+ " AND NoTagihan = " + NoTagihan
				+ " ORDER BY TglPelunasan DESC"
				;
