...
namespace ISC064.COLLECTION
{
	public partial class LogPk : System.Web.UI.Page
	{
		...
		{                        
            string Tb = "";
            if (Request.QueryString["tb"].ToString() == "" || Request.QueryString["tb"].ToString() == null)
            {
                tb.Visible = true;
                Tb = tb.SelectedValue;
            }
			...
			for(int i=0;i<rs.Rows.Count;i++)
			{
                ...
                c.Text = Db.SingleString("SELECT Nama FROM " + Mi.DbPrefix + "SECURITY..USERNAME WHERE Userid = '" + rs.Rows[i]["UserID"].ToString() + "'");
				c.Wrap = false;
				r.Cells.Add(c);
