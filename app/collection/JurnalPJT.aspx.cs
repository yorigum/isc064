...
{
	public partial class JurnalPJT : System.Web.UI.Page
	{
		...
		private void Fill()
		{
			string strSql = "SELECT *"
				+ ",(SELECT Nama FROM " + Mi.DbPrefix + "SECURITY..USERNAME WHERE UserID = MS_PJT_JURNAL.UserID) AS Nama"
				+ " FROM MS_PJT_JURNAL"
				+ " WHERE NoPJT = '" + NoPJT + "'"
				+ " ORDER BY JurnalID";
			...
			for(int i=0;i<rs.Rows.Count;i++)
			{
				...
				if(System.IO.File.Exists(Request.PhysicalApplicationPath
					+ "JurnalPJT\\" + rs.Rows[i]["JurnalID"] + ".jpg"))
					c.Text = "<a href=\"javascript:popGambar('JurnalPJT/"+rs.Rows[i]["JurnalID"]+".jpg')\">View</a>";
				else
					c.Text = "&nbsp;";
				r.Cells.Add(c);
