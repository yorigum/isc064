...
{
    public partial class SkemaKomisiEdit : System.Web.UI.Page
    {
        ...
        {
            DataTable rs = Db.Rs("SELECT * FROM REF_SKOM_DETAIL WHERE NoSkema = " + Nomor);
            if (rs.Rows.Count > 0)
            {
        ...
        {
            DataTable rs = Db.Rs("SELECT * FROM REF_SKOM_DETAIL2 WHERE NoSkema = " + Nomor);
            if (rs.Rows.Count > 0)
            {
                for (int i = 1; i <= rs.Rows.Count; i++)
                {
        ...
        {
            if (Save()) Response.Redirect("SkemaKomisiEdit.aspx?Nomor=" + Nomor + "&done=1");
        }
        ...
        private void UpdateSkema()
        {
            ...
            //Delete all Detail
            Db.Execute("DELETE FROM REF_SKOM_DETAIL WHERE NoSkema = " + Nomor);
            Db.Execute("DELETE FROM REF_SKOM_DETAIL2 WHERE NoSkema = " + Nomor);
        }
