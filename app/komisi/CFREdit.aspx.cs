...
{
    public partial class CFREdit : System.Web.UI.Page
    {
        ...
        {
            DataTable rs = Db.Rs("SELECT * FROM MS_KOMISI_CFR_DETAIL WHERE NoCFR = '" + Nomor + "'");
            if (rs.Rows.Count > 0)
            {
                for (int i = 0; i < rs.Rows.Count; i++)
                {
                    ...
                    c = new TableCell();
                    c.Text = Cf.Num(rs.Rows[i]["NilaiPPH"]);
                    c.CssClass = "right";
                    tr.Cells.Add(c);
        ...
        {
            if (Save()) Response.Redirect("CFREdit.aspx?Nomor=" + Nomor + "&done=1");
        }
