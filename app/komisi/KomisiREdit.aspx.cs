...
{
    public partial class KomisiREdit : System.Web.UI.Page
    {
        ...
        {
            DataTable rs = Db.Rs("SELECT * FROM MS_KOMISIR_DETAIL WHERE NoKomisiR = '" + Nomor + "'");
            if (rs.Rows.Count > 0)
            {
                for (int i = 0; i < rs.Rows.Count; i++)
                {
                    string Cust = "", NoKontrak = "", NoUnit = "";
                    string Sales = Db.SingleString("SELECT ISNULL(NamaAgent,'') FROM MS_KOMISI_TERM WHERE NoKomisi = '" + rs.Rows[i]["NoKomisi"].ToString() + "' AND SN = " + Convert.ToInt32(rs.Rows[i]["SN_KomisiTermin"]));
        ...
        {
            if (Save()) Response.Redirect("KomisiREdit.aspx?Nomor=" + Nomor + "&done=1");
        }
