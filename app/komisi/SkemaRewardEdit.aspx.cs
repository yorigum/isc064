...
{
    public partial class SkemaRewardEdit : System.Web.UI.Page
    {
        ...
        {
            DataTable rs = Db.Rs("SELECT * FROM REF_SKOM_REWARD_DETAIL WHERE NoSkema = " + Nomor);
            if (rs.Rows.Count > 0)
            {
                for (int i = 1; i <= rs.Rows.Count; i++)
                {
                    DropDownList lvlsales = (DropDownList)list.FindControl("lvlsales_" + i);
                    TextBox penjualan = (TextBox)list.FindControl("penjualan_" + i);
                    TextBox reward = (TextBox)list.FindControl("reward_" + i);

                    lvlsales.SelectedValue = rs.Rows[i - 1]["SalesLevel"].ToString();
                    penjualan.Text = Cf.Num(rs.Rows[i - 1]["Penjualan"]);
                    reward.Text = rs.Rows[i - 1]["Reward"].ToString();
                }
            }
        }
        private void FillTable2()
        {
            DataTable rs = Db.Rs("SELECT * FROM REF_SKOM_REWARD_DETAIL2 WHERE NoSkema = " + Nomor);
            if (rs.Rows.Count > 0)
            {
                for (int i = 1; i <= rs.Rows.Count; i++)
                {
                    ...
                    TextBox reward = (TextBox)list.FindControl("rewardd_" + i);
        ...
        private bool valid()
        {
            ...
            int cek = Db.SingleInteger("SELECT COUNT(*) FROM REF_SKOM_REWARD WHERE SalesTipe = " + Convert.ToInt32(tipe.SelectedValue)
                + " AND NoSkema != " + Nomor
                + " AND Inaktif = 0"
                + " AND "
                + " ("
        ...
        protected bool validform1
        {
            get
            {
                ...
                for (int i = 1; i <= Baris; i++)
                {
                    ...
                    if (penjualan.Text + reward.Text != "")
                    {
                        bool xx = true;
                        if (!Cf.isMoney(penjualan))
                        {
                            x = xx = false;
                            Cf.MarkError(penjualan);
                        }
        ...
        {
            if (Save()) Response.Redirect("SkemaRewardEdit.aspx?Nomor=" + Nomor + "&done=1");
        }
        ...
        private void UpdateSkema()
        {
            ...
            Db.Execute("EXEC spSkomRewardEdit"
                + "  " + Nomor
                + ",'" + Nama + "'"
                + ",'" + Inaktif + "'"
                + ", " + SalesTipe
            ...
            //Delete all Detail
            Db.Execute("DELETE FROM REF_SKOM_REWARD_DETAIL WHERE NoSkema = " + Nomor);
            Db.Execute("DELETE FROM REF_SKOM_REWARD_DETAIL2 WHERE NoSkema = " + Nomor);
        }
        private void UpdateSkemaDetail()
        {
            ...
            foreach (var r in list.Controls)
            {
                ...
                if (penjualan.Text + reward.Text != "")
                {
                    ...
                    decimal Penjualan = Convert.ToDecimal(penjualan.Text);
                    string Reward = Cf.Str(reward.Text);

                    Db.Execute("EXEC " + Mi.DbPrefix + "MARKETINGJUAL..spSkomRewardTambah "
                        + Nomor
                        + ", " + SalesLevel
                        + ", " + Penjualan
                        + ",'" + Reward + "'"
        ...
        private void UpdateSkemaDetail2()
        {
            ...
            foreach (var r in list2.Controls)
            {
                ...
                if (bawah.Text != "" && atas.Text != "" && reward.Text != "")
                {
                    int SalesLevel = Convert.ToInt32(lvlsales.SelectedValue);
                    ...
                    Db.Execute("EXEC " + Mi.DbPrefix + "MARKETINGJUAL..spSkomRewardTambah2 "
                        + Nomor
                        + ", " + SalesLevel
                        + ",'" + TipeTarget + "'"
                        + ", " + TargetBawah
        ...
        void EditLoger(string Nomor)
        {
            ...
            DataTable rsSkemaBef = Db.Rs("SELECT [Nama] as [Nama Komisi] "
                                  ...
                                  + " ,[Sampai]"
                                  + " ,[Rumus]"
                                  + " FROM REF_SKOM_REWARD where NoSkema = " + Nomor);

            DataTable rsDetailBef = Db.Rs("SELECT "
                    + "CONVERT(VARCHAR,SN) + '.   ' + (SELECT Nama FROM REF_AGENT_LEVEL WHERE LevelID = SalesLevel) + '   ' + CONVERT(VARCHAR,Penjualan,1) + '   ' + Reward "
                    + "FROM REF_SKOM_REWARD_DETAIL WHERE NoSkema = '" + Nomor + "' ORDER BY SN");

            DataTable rsDetailBef2 = Db.Rs("SELECT "
                    + "CONVERT(VARCHAR,SN) + '.   ' + (SELECT Nama FROM REF_AGENT_LEVEL WHERE LevelID = SalesLevel) + '   ' + CONVERT(VARCHAR,TargetBawah,1) + ' - ' + CONVERT(VARCHAR,TargetAtas,1) + ' (' + TipeTarget + ')   ' + Reward "
                    + "FROM REF_SKOM_REWARD_DETAIL2 WHERE NoSkema = '" + Nomor + "' ORDER BY SN");
