...
{
    public partial class SkemaRewardRegis : System.Web.UI.Page
    {
            ...
            if (!Page.IsPostBack)
            {
                if (Request.QueryString["done"] != null)
                    feed.Text = "<img src='/Media/db.gif' align=absmiddle> "
                        + "<a href=\"javascript:popSkomReward('" + Request.QueryString["done"] + "')\">"
        ...
        protected void tambah2(int index)
        {
            ...
            l.CssClass = "btn btn-cal";
            StringBuilder x = new StringBuilder();
            x.Append("ClearSkema2('bawah_" + index + "','atas_" + index + "','rewardd_" + index + "');");
            l.Attributes["onclick"] = x.ToString();
            c.Controls.Add(l);
            r.Cells.Add(c);
        ...
        protected void Save()
        {
            ...
            Db.Execute("EXEC spSkomRewardBaru"
                + " '" + Nama + "'"
                + ", " + SalesTipe
                + ",'" + Dari + "'"
                + ",'" + Sampai + "'"
            ...
            int Nomor = Db.SingleInteger("SELECT TOP 1 NoSkema FROM REF_SKOM_REWARD"
                + " ORDER BY NoSkema DESC"
                );

            Db.Execute("UPDATE REF_SKOM_REWARD SET "
            ...
            DataTable rsDetail = Db.Rs("SELECT "
                ...
                + " + CONVERT(VARCHAR, Penjualan, 1) + '  ' "
                + " + Reward "
                + " FROM " + Mi.DbPrefix + "MARKETINGJUAL..REF_SKOM_REWARD_DETAIL WHERE NoSkema = " + Nomor);
            ...
            DataTable rsDetail2 = Db.Rs("SELECT "
                ...
                + " + Reward "
                + " FROM " + Mi.DbPrefix + "MARKETINGJUAL..REF_SKOM_REWARD_DETAIL2 WHERE NoSkema = " + Nomor);
            ...
            Response.Redirect("SkemaReward.aspx?done=" + Nomor);
        }
