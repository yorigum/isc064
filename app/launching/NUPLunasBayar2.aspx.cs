...
namespace ISC064.LAUNCHING
{
    public partial class NUPLunasBayar2 : System.Web.UI.Page
    {
        ...
        private bool valid()
        {
            ...
            //if (carabayar.SelectedValue == "KK" && nokk.Text == "")
            //{
            //    x = false;
            //    if (s == "") s = nokk.ID;
            //    nokkc.Text = "Kosong";
            ...
            int c = Db.SingleInteger("SELECT COUNT(*) FROM MS_NUP_PELUNASAN WHERE NoNUP='" + NoNUP + "' AND PelunasanKe=2");
            if (c != 0)
            {
                x = false;
        ...
        {
            decimal nilaiPertama = Db.SingleDecimal("SELECT NilaiBayar FROM MS_NUP_PELUNASAN WHERE NoNUP='" + NoNUP + "' AND PelunasanKe=1");

            if (nilaiPertama == 2000000)
            {
                string v = "3000000";
                string t = "3,000,000";
                valueNUP.Items.Add(new ListItem(t, v));
            }
            else
            {
                string v = "2000000";
        ...
        private void fill()
        {
            ...
            if (rsNUP.Rows.Count != 0)
            {
                ...
                decimal bayaran = Db.SingleDecimal("SELECT ISNULL(NilaiBayar,0) FROM MS_NUP_PELUNASAN WHERE NoNUP='" + rsNUP.Rows[0]["NoNUP"].ToString() + "' AND PelunasanKe=1");
        ...
        protected void save_Click(object sender, System.EventArgs e)
        {
            if (valid())
            {
                ...
                Db.Execute("UPDATE MS_NUP_PELUNASAN SET FlagUntukBayar=1 WHERE NoNUP='" + NoNUP + "' AND PelunasanKe=1");
                Db.Execute("UPDATE MS_NUP_PELUNASAN SET FlagUntukBayar=1 WHERE NoNUP='" + NoNUP + "' AND PelunasanKe=2");
               ...
               + "<br>***PEMBAYARAN NUP:<br>";
                ...
                Response.Redirect("NUPLunasBayarFin.aspx?No=" + NoNUP + "&project=" + Project + "&Tipe=" + Tipe);
