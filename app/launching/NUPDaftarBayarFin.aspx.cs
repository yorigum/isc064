    ...
    public partial class NUPDaftarBayarFin : System.Web.UI.Page
	{
		...
		private void Fill()
		{
            ...
            if (dtNUP.Rows.Count > 0)
            {
                ...
                string notts = Db.SingleString("SELECT NoTTS FROM MS_NUP_PELUNASAN WHERE NoNUP='" + NoNUP + "'");

                customer.Text = namaCS;
                agent.Text = namaAG;
                asp.HRef = "javascript:openPopUp('/marketingjual/PrintTTS.aspx?NoTTS=" + notts + "','920','600')";
