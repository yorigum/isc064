		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(!Page.IsPostBack)
			{
				...
				if(persen!=""&&ket!="")
				{
					...
					try { persen1.Text = p[0]; } catch{}
					try { persen2.Text = p[1]; } catch{}
					try { persen3.Text = p[2]; } catch{}
					try { persen4.Text = p[3]; } catch{}
					try { persen5.Text = p[4]; } catch{}
					try { persen6.Text = p[5]; } catch{}
					try { persen7.Text = p[6]; } catch{}
					try { persen8.Text = p[7]; } catch{}
					try { persen9.Text = p[8]; } catch{}
					try { persen10.Text = p[9]; } catch{}

//					try { persen1.Text = (Convert.ToDecimal(p[0]) * (decimal)-1).ToString(); } catch{}
//					try { persen2.Text = (Convert.ToDecimal(p[1]) * (decimal)-1).ToString(); } catch{}
//					try { persen3.Text = (Convert.ToDecimal(p[2]) * (decimal)-1).ToString(); } catch{}
//					try { persen4.Text = (Convert.ToDecimal(p[3]) * (decimal)-1).ToString(); } catch{}
//					try { persen5.Text = (Convert.ToDecimal(p[4]) * (decimal)-1).ToString(); } catch{}
//					try { persen6.Text = (Convert.ToDecimal(p[5]) * (decimal)-1).ToString(); } catch{}
//					try { persen7.Text = (Convert.ToDecimal(p[6]) * (decimal)-1).ToString(); } catch{}
//					try { persen8.Text = (Convert.ToDecimal(p[7]) * (decimal)-1).ToString(); } catch{}
//					try { persen9.Text = (Convert.ToDecimal(p[8]) * (decimal)-1).ToString(); } catch{}
//					try { persen10.Text = (Convert.ToDecimal(p[9]) * (decimal)-1).ToString(); } catch{}
		...
		protected void ok_Click(object sender, System.EventArgs e)
		{
			if(valid())
			{
				...
				if(persen1.Text!="") persen.Append("+"+persen1.Text);
				if(persen2.Text!="") persen.Append("+"+persen2.Text);
				if(persen3.Text!="") persen.Append("+"+persen3.Text);
				if(persen4.Text!="") persen.Append("+"+persen4.Text);
				if(persen5.Text!="") persen.Append("+"+persen5.Text);
				if(persen6.Text!="") persen.Append("+"+persen6.Text);
				if(persen7.Text!="") persen.Append("+"+persen7.Text);
				if(persen8.Text!="") persen.Append("+"+persen8.Text);
				if(persen9.Text!="") persen.Append("+"+persen9.Text);
				if(persen10.Text!="") persen.Append("+"+persen10.Text);
				if(persen.Length!=0)
...
//					persen = persen.Remove(0,1);
				...
				RegisterStartupScript(
					...
					+ " dialogArguments.document.getElementById('"+Request.QueryString["d1"]+"').value='"+persen+"';"
					+ " dialogArguments.document.getElementById('"+Request.QueryString["d2"]+"').value='"+ket+"';"
					+ " dialogArguments.recal();"
//						+ " dialogArguments.document.getElementById('gross')"
//						+ ",dialogArguments.document.getElementById('disc')"
//						+ ",dialogArguments.document.getElementById('nilai'));"
					+ " window.close();"
					+ "</script>"
					);
			}
