...
public class ClosingHub : Hub {
    ...
    public override Task OnConnected()
    {
        ...
        {
            bool AdaKoneksi = Db.SingleInteger("SELECT COUNT(*) FROM ISC064A_MARKETINGJUAL..MS_UNIT_CLOSING WHERE "
                + " NoStock = '" + NoStock + "' "
                + " AND Connected = 1" // Masih aktif
                + " AND UserID <> '" + UserID + "'"
                ) > 0;
            bool SempatAktif = Db.SingleInteger("SELECT COUNT(*) FROM ISC064A_MARKETINGJUAL..MS_UNIT_CLOSING WHERE "
                 + " NoStock = '" + NoStock + "' "
                 + " AND Connected = 0"
                 + " AND UserID = '" + UserID + "'"
                 ) > 0;
            ...
            {
                Db.Execute("INSERT INTO ISC064A_MARKETINGJUAL..MS_UNIT_CLOSING "
    ...
    public override Task OnDisconnected(bool stopCalled)
    {
        ...
        Db.Execute("DELETE FROM ISC064A_MARKETINGJUAL..MS_UNIT_CLOSING WHERE NoStock = '" + NoStock + "' AND UserID = '" + UserID + "'");
        //Db.Execute("UPDATE ISC064A_MARKETINGJUAL..MS_UNIT_CLOSING "
