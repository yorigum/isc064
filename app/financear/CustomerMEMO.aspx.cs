...
{
	public partial class CustomerMEMO : System.Web.UI.Page
	{
		...
		private void FillTable()
		{
			string strSql = "SELECT *"
				+ ",(SELECT Nama FROM " + Mi.DbPrefix + "SECURITY..USERNAME WHERE UserID = MS_MEMO.UserID) AS Nama"
				+ ",CASE CaraBayar"
				+ "		WHEN 'TN' THEN 'TUNAI'"
			...
			for(int i=0;i<rs.Rows.Count;i++)
			{
				...
				c = new TableCell();
				c.Text = "<a href='MEMOEdit.aspx?NoMEMO="+rs.Rows[i]["NoMEMO"]+"' "+strike+">"
					+ rs.Rows[i]["NoMEMO"].ToString().PadLeft(7,'0') + "</a>"
					+ "<br><i>"+status+"</i>"
					+ bkm;
				r.Cells.Add(c);
				...
				c = new TableCell();
				c.Text = Cf.Day(rs.Rows[i]["TglMEMO"])
