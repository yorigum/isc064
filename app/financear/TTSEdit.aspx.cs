    ...
    public partial class TTSEdit : System.Web.UI.Page
    {
        ...
        private bool validKwitansi()
        {
            ...
            if (Cf.isEmpty(nokontrak))
            {
                ...
                alasan.Text = "No. TTS ini belum memiliki No. Kontrak";
            }
            else
                nokontrakc.Text = "";
            ...
            if (!x)
                Js.Alert(
                    this
                    , "No. TTS ini belum memiliki No. Kontrak.\\n\\n"
        ...
        protected void btnkw_Click(object sender, EventArgs e)
        {
            validKwitansi();
            {
                int count = Db.SingleInteger("SELECT COUNT(*) FROM MS_TUNGGAKAN WHERE Ref = (SELECT Ref FROM MS_TTS WHERE NoTTS = '" + NoTTS + "')");
                if (count > 0)
                {
                    //int cek = Db.SingleInteger(" select COUNT(*) FROM " + Mi.DbPrefix + "MARKETINGJUAL..ms_pelunasan a join ms_tunggakan b on a.NoKontrak = b.Ref "
                    //    + " WHERE NoKontrak = (SELECT Ref FROM MS_TTS WHERE NoTTS = '" + NoTTS + "') AND a.NoTTS = '" + NoTTS + "'"
                    //    + " AND a.NoTagihan IN (SELECT NoTagihan FROM MS_TUNGGAKAN_DETIL WHERE NoTunggakan = b.NoTunggakan)");

                    //if(cek > 0)
                    //{
                    //    int NoTunggakan = Db.SingleInteger("select NoTunggakan FROM " + Mi.DbPrefix + "MARKETINGJUAL..ms_pelunasan a join ms_tunggakan b on a.NoKontrak = b.Ref "
                    //    + " WHERE NoKontrak = (SELECT Ref FROM MS_TTS WHERE NoTTS = '" + NoTTS + "') AND a.NoTTS = '" + NoTTS + "'"
                    //    + " AND a.NoTagihan IN (SELECT NoTagihan FROM MS_TUNGGAKAN_DETIL WHERE NoTunggakan = b.NoTunggakan)");

                    Db.Execute("UPDATE MS_TUNGGAKAN SET Status='S' WHERE Ref=(SELECT Ref FROM MS_TTS WHERE NoTTS = '" + NoTTS + "')");
                    //}
                }
                DendaIlang(NoTTS);
            }
        ...
        private void Fill()
        {
            ...
            else
            {
                ...
                tglfp.Text = Cf.Day(rs.Rows[0]["TglFP"]);
                ...
                printBKM.HRef = "PrintBKM.aspx?NoTTS=" + NoTTS + "&project=" + Project;
                printFPS.HRef = "PrintFPS.aspx?NoTTS=" + NoTTS + "&project=" + Project;

                //identitas customer
                nokontrak.Text = rs.Rows[0]["Ref"].ToString();
                ...
                if (rs.Rows[0]["CaraBayar"].ToString() == "BG")
                {
                    ...
                    tgljtbg.Text = Cf.Day(rs.Rows[0]["TglJTBG"]);
                ...
                else if (stat == "POST")
                {
                    ...
                    bkminfo.Text = rs.Rows[0]["NoBKM2"].ToString().PadLeft(7, '0')
                ...
                if (rs.Rows[0]["Tipe"].ToString() != "TENANT")
                    alokasi.InnerHtml = "<a onclick=\"popEditPelunasan('" + rs.Rows[0]["Tipe"] + "','" + rs.Rows[0]["Ref"] + "')\">"
                        + "<b>Alokasi Pelunasan</b></a>";

                manualtts.Text = rs.Rows[0]["ManualTTS"].ToString();
                manualbkm.Text = rs.Rows[0]["ManualBKM"].ToString();

                ddlAcc.SelectedValue = rs.Rows[0]["Acc"].ToString() + ";" + rs.Rows[0]["SubID"].ToString();
                ...
                //warning untuk TTS yang alokasi / no. kontrak-nya belon ada...  Req. Panahome
            }
        }
