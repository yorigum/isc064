...
{
	public partial class KasMasukEdit : System.Web.UI.Page
	{
		...
		private void Fill()
		{
			btnlog.Attributes["onclick"] = "location.href='LogPk.aspx?Tb=MS_KASMASUK_LOG&Pk="+NoVoucher.ToString().PadLeft(5,'0')+"'";
			
			printKM.HRef = "PrintKM.aspx?NoVoucher="+NoVoucher;
		...
		private bool Save()
		{
			if(valid())
			{
				...
				Db.Execute("EXEC spKasMasukEdit"
					+ "  " + NoVoucher
					+ ",'" + Tgl + "'"
					+ ",'" + Acc + "'"
					+ ",'" + CaraBayar + "'"
		...
		{
			if(Save()) Response.Redirect("KasMasukEdit.aspx?done=1&NoVoucher="+NoVoucher);
		}
