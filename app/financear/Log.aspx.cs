...
namespace ISC064.FINANCEAR
{
	public partial class Log : System.Web.UI.Page
	{
        ...
        private void Fill2()
        {
            ...
            //int pk = Db.SingleInteger("SELECT * FROM " +tb.SelectedValue+ " WHERE 1=1 AND CONVERT(varchar,Tgl,112) >= '" + Cf.Tgl112(Dari) + "'"
            //+ " AND CONVERT(varchar,Tgl,112) <= '" + Cf.Tgl112(Sampai) + "'"+Project+"" );

            string nav = ",'<a href=\"javascript:popLogDetil2('''+ CONVERT(varchar(50),LogID) +''' , ''" + tb.SelectedValue + "'', ''" + tb.SelectedValue + "'', '''+Pk+''')\">' + Aktivitas + '</a>'";

            string popup = "";
            if (href.SelectedItem.Value == "MS_TTS_LOG")
            {
                popup = ",'<a href=\"javascript:popEditTTS('''+Pk+''')\">' + Pk +  '</a>'";
            }
            else if (href.SelectedItem.Value == "MS_ANONIM_LOG")
            {
                popup = ",'<a href=\"javascript:popEditTA('''+Pk+''')\">' + Pk +  '</a>'";
            }
            else if (href.SelectedItem.Value == "REF_ACC_LOG")
            {
                popup = ",'<a href=\"javascript:popEditAcc('''+Pk+''')\">' + Pk +  '</a>'";
            }
            else if (href.SelectedItem.Value == "MS_KASMASUK_LOG")
            {
                popup = ",'<a href=\"javascript:popEditKasMasuk('''+Pk+''')\">' + Pk +  '</a>'";
            }
            else if (href.SelectedItem.Value == "MS_KASKELUAR_LOG")
            {
                popup = ",'<a href=\"javascript:popEditKasKeluar('''+Pk+''')\">' + Pk +  '</a>'";
            }
            else if (href.SelectedItem.Value == "MS_CASHBACK_LOG")
            {
                popup = ",'<a href=\"javascript:popEditCB('''+Pk+''')\">' + Pk +  '</a>'";
            }
            else
                popup = ",''+Pk+''";
		...
		private void Fill()
		{
			...
			for(int i=0;i<rs.Rows.Count;i++)
			{
				...
				c = new TableCell();
				c.Text = "<a show-modal='#ModalPopUp' modal-url='LogDetil.aspx?LogID="+rs.Rows[i]["LogID"]+"&tb="+tb.SelectedValue+"&sumber="+tb.SelectedItem.Text+"&pk="+ rs.Rows[i]["Pk"].ToString()+"'>"
