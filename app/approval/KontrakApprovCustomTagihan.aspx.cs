...
namespace ISC064.APPROVAL
{
    ...
    public partial class KontrakApprovCustomTagihan : System.Web.UI.Page
    {
        ...
        private bool validuser()
        {
            ...
            if (Db.SingleInteger("SELECT COUNT(Lvl) FROM " + Mi.DbPrefix + "SECURITY..REF_APPROVAL WHERE Tipe = 7 AND Project IN (" + Projected + ") AND UserID = '" + Act.UserID + "'") == 0)
            {
                x = false;
            }
            ...
            if (!Page.IsPostBack)
            {
                if (Request.QueryString["done"] == "1")
                    feed.Text = "<img src='/Media/db.gif' align=absmiddle> "
                        + "Approval Customize Tagihan Kontrak Berhasil...";
                else if (Request.QueryString["done"] == "2")
                    feed.Text = "<img src='/Media/db.gif' align=absmiddle> "
                        + "Reject Customize Tagihan Kontrak Berhasil...";
            }
        }
        ...
        protected void Fill()
        {
            ...
            DataTable rs2 = Db.Rs("SELECT Lvl FROM ISC064_SECURITY..REF_APPROVAL a WHERE Tipe = 7 AND Project = '" + project.SelectedValue + "' AND UserID = '" + Act.UserID + "'");
            if (rs2.Rows.Count != 0)
            {
                ...
                string nav = "'<a href=KontrakApprovCustomTagihan2.aspx?NoKontrak=''' + A.NoKontrak + '''&NoApproval=''' + e.NoApproval + '''&Level=" + Convert.ToInt16(rs2.Rows[0]["Lvl"]) + ">Next</a><br>'";
