...
namespace ISC064.APPROVAL
{
    public partial class KontrakApprovBatal : System.Web.UI.Page
    {
            ...
            if (!Page.IsPostBack)
            {
                if (Request.QueryString["done"] == "1")
                    feed.Text = "<img src='/Media/db.gif' align=absmiddle> "
                        + "Approval Batal Kontrak Berhasil...";
                else if (Request.QueryString["done"] == "2")
                    feed.Text = "<img src='/Media/db.gif' align=absmiddle> "
                        + "Reject Batal Kontrak Berhasil...";
            }
        }
        ...
        protected void Fill()
        {
            ...
            DataTable rs2 = Db.Rs("SELECT Lvl FROM " + Mi.DbPrefix + "SECURITY..REF_APPROVAL a WHERE Tipe = 3 AND Project = '" + project.SelectedValue + "' AND UserID = '" + Act.UserID + "'");
            if (rs2.Rows.Count != 0)
            {
                ...
                string nav = "'<a href=KontrakApprovBatal2.aspx?NoKontrak=''' + A.NoKontrak + '''&NoApproval=''' + e.NoApproval + '''&Level=" + Convert.ToInt16(rs2.Rows[0]["Lvl"]) + ">Next</a><br>'";
