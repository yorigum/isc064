...
{
    public partial class ApprovalTagihanReschedule : System.Web.UI.Page
    {
            ...
            if (!Page.IsPostBack)
            {
                if (Request.QueryString["done"] == "1")
                    feed.Text = "<img src='/Media/db.gif' align=absmiddle> "
                        + "Approval Reschedule Tagihan Berhasil...";
                else if (Request.QueryString["done"] == "2")
                {
                    feed.Text = "<img src='/Media/db.gif' align=absmiddle> "
                        + "Reject Reschedule Tagihan Berhasil...";
                }
            }
        }
        ...
        private bool validuser()
        {
            ...
            if (Db.SingleInteger("SELECT COUNT(Lvl) FROM " + Mi.DbPrefix + "SECURITY..REF_APPROVAL WHERE Tipe = 6 AND Project IN (" + Projected + ") AND UserID = '" + Act.UserID + "'") == 0)
            {
                x = false;
            }
        ...
        protected void Fill()
        {
            ...
            DataTable rs2 = Db.Rs("SELECT Lvl FROM " + Mi.DbPrefix + "SECURITY..REF_APPROVAL a WHERE Tipe = 6 AND Project = '" + project.SelectedValue + "' AND UserID = '" + Act.UserID + "'");
            ...
            string nav = "'<a href=ApprovalTagihanReschedule2.aspx?NoKontrak=''' + A.NoKontrak + '''&NoApproval=''' + e.NoApproval + '''&Level=" + Convert.ToInt16(rs2.Rows[0]["Lvl"]) + ">Next</a><br>'";
