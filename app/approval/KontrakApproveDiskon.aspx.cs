...
namespace ISC064.APPROVAL
{
    ...
    public partial class KontrakApproveDiskon : System.Web.UI.Page
    {
        ...
        private bool validuser()
        {
            ...
            if (Db.SingleInteger("SELECT COUNT(Lvl) FROM " + Mi.DbPrefix + "SECURITY..REF_APPROVAL WHERE Tipe = 4 AND Project IN (" + Projected + ") AND UserID = '" + Act.UserID + "'") == 0)
            {
                x = false;
            }
            ...
            if (!Page.IsPostBack)
            {
                if (Request.QueryString["done"] == "1")
                    feed.Text = "<img src='/Media/db.gif' align=absmiddle> "
                        + "Approval Diskon Kontrak Berhasil...";
                else if (Request.QueryString["done"] == "2")
                    feed.Text = "<img src='/Media/db.gif' align=absmiddle> "
                        + "Reject Diskon Kontrak Berhasil...";
            }
        }
