...
namespace ISC064.SECURITY
{
	public partial class Mapping : System.Web.UI.Page
	{
		...
		private void Fill()
		{
			...
			DataTable rs = Db.Rs("SELECT * FROM PAGE WHERE 1=1 " + addq + " ORDER BY Modul, Nama");
			...
			for(int i=0;i<rs.Rows.Count;i++)
			{
				...
				c.Text = "<a style='font:8pt' "
					+ "href=\"javascript:mapping('"+rs.Rows[i]["Halaman"].ToString().Replace("\\", "\\\\") + "')\">"
					+ namaFile + "</a>";
				r.Cells.Add(c);
		...
		private string GetSecLevel(string Halaman)
		{
			...
			DataTable rsSl = Db.Rs("SELECT Kode FROM PAGESEC WHERE Halaman = '"+Halaman+"' ORDER BY Kode");
