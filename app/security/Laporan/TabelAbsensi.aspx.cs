...
namespace ISC064.SECURITY.Laporan
{
	public partial class TabelAbsensi : System.Web.UI.Page
	{
		...
		private void init()
		{
			...
			rs = Db.Rs("SELECT DISTINCT UserID, Nama FROM LOGIN ORDER BY Nama, UserID");
			...
			rs = Db.Rs("SELECT DISTINCT IP FROM LOGIN ORDER BY IP");
			for(int i=0;i<rs.Rows.Count;i++)
        ...
        protected void pdf_Click(object sender, System.EventArgs e)
        {
            ...
            string link = Mi.PathAlamatWeb + "security/LaporanPDF/PDFTabelAbsensi.aspx?id=" + rs.Rows[0]["AttachmentID"] + "&user=" + user.SelectedValue + "&ip=" + ip.SelectedValue + "&seclevel=" + SecLevel + "";
		...
		private void Header()
		{
			...
			Rpt.SubJudul(x
				, "Tanggal Masuk : " + Cf.Day(dari.Text) + " s/d " + Cf.Day(sampai.Text)
				);
