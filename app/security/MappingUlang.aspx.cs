...
{
    public partial class MappingUlang : System.Web.UI.Page
    {
        ...
        {
            Db.Execute("UPDATE PAGE SET Baru = 0");

            //ambil folder /app
            string[] x = System.IO.Directory.GetDirectories(app);
            for (int i = 0; i <= x.GetUpperBound(0); i++)
            {
                PrintDir(x[i], app); //directory
            }

            CustomMapping(); //mapping.txt

            Db.Execute("DELETE FROM PAGE WHERE Baru = 0");

            Response.Redirect("Mapping.aspx?done=1");
        }
        ...
        private void PrintFile(string[] files)
        {
            for (int i = 0; i <= files.GetUpperBound(0); i++)
            {
                ...
                switch (ctrl)
                {
                    ...
                    case "2": //Free Akses
                        Map(file, modul, judul);
                        Db.Execute("UPDATE PAGE SET FreeAkses = 1 WHERE Halaman = '" + file + "'");
                        break;
        ...
        {
            string mappingtxt = app + "\\security\\Mapping.txt";
        ...
        {
            if (Db.SingleInteger("SELECT COUNT(*) FROM PAGE WHERE Halaman = '" + file + "'") == 0)
                Db.Execute("INSERT INTO PAGE ("
                    + " Halaman"
                    + ",Modul"
                    + ",Nama"
                    + ",Baru"
