    ...
    public partial class HeadKontrak : System.Web.UI.UserControl
    {
        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                string p = Db.SingleString("SELECT ISNULL((SELECT TOP 1 NoKontrak FROM MS_KONTRAK WHERE CaraBayar='KPA' AND NoKontrak < '" + NoKontrak + "' AND Project IN (" + Act.ProjectListSql + ") ORDER BY NoKontrak DESC),'')");
                string n = Db.SingleString("SELECT ISNULL((SELECT TOP 1 NoKontrak FROM MS_KONTRAK WHERE CaraBayar='KPA' AND NoKontrak > '" + NoKontrak + "' AND Project IN (" + Act.ProjectListSql + ") ORDER BY NoKontrak ASC),'')");
                ...
                else
                {
                    ...
                    unit.Text = "<a href='../marketingjual/UnitInfo.aspx?NoStock=" + rs.Rows[0]["NoStock"] + "'>"
                        + rs.Rows[0]["NoUnit"] + "</a>";
                    customer.Text = "<a href='../marketingjual/CustomerEdit.aspx?NoCustomer=" + rs.Rows[0]["NoCustomer"] + "'>"
                        + rs.Rows[0]["Cs"] + "</a>";
