namespace ISC064.LEGAL
{
    ...
    public partial class HeadPPJB : System.Web.UI.UserControl
{
        protected void Page_Load(object sender, System.EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                string p = Db.SingleString("SELECT ISNULL((SELECT TOP 1 NoKontrak FROM MS_PPJB WHERE NoKontrak < '" + NoKontrak + "' AND (SELECT COUNT(Project) FROM MS_KONTRAK WHERE NoKontrak = MS_PPJB.NoKontrak AND Project IN(" + Act.ProjectListSql + ")) > 0 ORDER BY NoKontrak DESC),'')");
                string n = Db.SingleString("SELECT ISNULL((SELECT TOP 1 NoKontrak FROM MS_PPJB WHERE NoKontrak > '" + NoKontrak + "' AND (SELECT COUNT(Project) FROM MS_KONTRAK WHERE NoKontrak = MS_PPJB.NoKontrak AND Project IN(" + Act.ProjectListSql + ")) > 0 ORDER BY NoKontrak ASC),'')");
