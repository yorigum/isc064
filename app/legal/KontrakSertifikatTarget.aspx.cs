...
{
    public partial class KontrakSertifikatTarget : System.Web.UI.Page
    {
        ...
        private bool validKontrak()
        {
            ...
            int a = Db.SingleInteger(
                    "SELECT COUNT(*) FROM MS_SERTIFIKAT WHERE NoKontrak = '" + NoKontrak + "' AND (StatusSertifikat=1 OR StatusSertifikat=2 OR StatusSertifikat=3)");
            if (a > 0)
                x = false;
        ...
        private bool valid()
        {
            ...
            if (!x)
                Js.Alert(
                    ...
                    + "4. Prosedur Sertifikat sudah dijalankan.\\n"
        ...
        private void Save()
        {
            ...
            if (c == 0)
            {
                ...
                + ", TglTargetSertifikat AS [Tgl Target Sertifikat]"
                + " FROM MS_SERTIFIKAT"
                + " WHERE NoKontrak = '" + NoKontrak + "'"
                );

                DateTime Tgl = Convert.ToDateTime(tbTgl.Text);
                string status = "S"; // B = Belum, S = Target, D = Proses, T = Sertifikat

                Db.Execute("INSERT INTO MS_SERTIFIKAT (NoKontrak,NoSertifikat,TglTargetSertifikat,StatusSertifikat)"
                    + "VALUES ('" + NoKontrak + "','','" + Tgl + "','" + status + "')");
            ...
            else
            {
                ...
                Db.Execute("UPDATE MS_SERTIFIKAT"
                    + " SET TglTargetSertifikat = '" + Convert.ToDateTime(tbTgl.Text) + "'"
            ...
            Response.Redirect("KontrakSertifikatTarget.aspx?done=" + NoKontrak);
        }
