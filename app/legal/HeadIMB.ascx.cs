namespace ISC064.LEGAL
{
    ...
    public partial class HeadIMB : System.Web.UI.UserControl
    {
        protected void Page_Load(object sender, System.EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                string p = Db.SingleString("SELECT ISNULL((SELECT TOP 1 NoKontrak FROM MS_IMB WHERE NoKontrak < '" + NoKontrak + "' ORDER BY NoKontrak DESC),'')");
                string n = Db.SingleString("SELECT ISNULL((SELECT TOP 1 NoKontrak FROM MS_IMB WHERE NoKontrak > '" + NoKontrak + "' ORDER BY NoKontrak ASC),'')");
