...
namespace ISC064.LEGAL
{
    ...
    public partial class KontrakIMB : System.Web.UI.Page
    {
        ...
        private bool valid()
        {
            ...
            int a = Db.SingleInteger(
                     "SELECT COUNT(*) FROM MS_IMB WHERE NoKontrak = '" + NoKontrak + "' AND NoIMB != ''  ");
            if (a > 0)
                x = false;
        ...
        private void Save()
        {
            ...
            if (c == 0)
            {
                ...
                sql = "INSERT INTO MS_IMB (NoKontrak, NoIMB, StatusIMB, KetIMB,TglIMB)"
                      + "VALUES('" + NoKontrak + "','" + NoIMB + "','" + status + "','" + keteranganimb + "','" + Tgl + "')";
                Db.Execute(sql);
                ...
                DataTable rs = Db.Rs("SELECT"
                                               ...
                                               + ", case when A.StatusIMB='S' then 'Target IMB' when A.StatusIMB='D' then 'Proses IMB' when A.StatusIMB='B' then 'Belum IMB' else 'IMB' end as [Status IMB]"
            ...
            Response.Redirect("KontrakIMB.aspx?done=" + NoKontrak);
        }
