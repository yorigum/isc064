...
{
    public partial class LapPenjualanApi : System.Web.UI.Page
    {
        ...
        private Table fillAll(DateTime Dari, DateTime Sampai, string strAdd)
        {
            ...
            for (int i = 0; i < rs1.Rows.Count; i++)
            {
                ...
                c1 = new TableCell();
                // decimal NominalBunga = Db.SingleDecimal("SELECT BungaNominal FROM ISC064_MARKETINGJUAL..MS_KONTRAK WHERE NoKontrak = '"++"' ");
        ...
        private Table fillBatal(DateTime Dari, DateTime Sampai, string Tipe, string strAdd)
        {
            ...
            for (int i = 0; i < rs2.Rows.Count; i++)
            {
                ...
                // nilai pembatalan
                decimal nBatal = Db.SingleDecimal("SELECT NilaiTagihan FROM ISC064_MARKETINGJUAL..MS_TAGIHAN WHERE NamaTagihan = 'BIAYA ADM. PEMBATALAN' AND NoKontrak='" + rs2.Rows[i]["NoKontrak"] + "'");
                c2 = new TableCell();
                ...
                //jumlah pembayaran
                c2 = new TableCell();
                decimal NilaiPembayaran = Db.SingleDecimal("SELECT ISNULL(SUM(NilaiPelunasan),0) AS TotalPembayaran FROM ISC064_MARKETINGJUAL..MS_PELUNASAN WHERE NoKontrak = '" + rs2.Rows[i]["NoKontrak"] + "'");
        ...
        private void grafik(string dari, string sampai)
        {
            ...
            for (int i = Convert.ToInt32(Convert.ToDateTime(dari).Month); i <= batasulang; i++)
            {
                ...
                int qbatalBulan = Db.SingleInteger("SELECT ISNULL(COUNT(*),0)"
                    + " FROM ISC064_MARKETINGJUAL..MS_KONTRAK a INNER JOIN ISC064_MARKETINGJUAL..MS_UNIT u ON a.NoUnit = u.NoUnit"
                ...
                int qallBulan = Db.SingleInteger("SELECT ISNULL(COUNT(*),0) FROM ISC064_MARKETINGJUAL..MS_KONTRAK a INNER JOIN ISC064_MARKETINGJUAL..MS_UNIT u ON a.NoUnit = u.NoUnit"
                ...
                int qnetBulan = Db.SingleInteger("SELECT ISNULL(COUNT(*),0) FROM ISC064_MARKETINGJUAL..MS_KONTRAK a INNER JOIN ISC064_MARKETINGJUAL..MS_UNIT u ON a.NoUnit = u.NoUnit"
                    + " WHERE TglKontrak >= '" + Convert.ToDateTime(daritgl) + "'"
                    + " AND TglKontrak <= '" + Convert.ToDateTime(sampaitgl) + "'"
                    //+ " AND a.Jenis IN (" + akt + ")"
                    + " AND a.Project = '" + Project + "'"
