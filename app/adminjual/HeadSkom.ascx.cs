	...
	public partial class HeadSkom : System.Web.UI.UserControl
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			if(!Page.IsPostBack)
			{
				...
				int p = Db.SingleInteger("SELECT ISNULL((SELECT TOP 1 Nomor FROM REF_SKOM WHERE Nomor < "+Nomor+" ORDER BY Nomor DESC),0)");
				int n = Db.SingleInteger("SELECT ISNULL((SELECT TOP 1 Nomor FROM REF_SKOM WHERE Nomor > "+Nomor+" ORDER BY Nomor ASC),0)");
				...
				string strSql = "SELECT Nomor,Nama FROM REF_SKOM WHERE Nomor = " + Nomor;
