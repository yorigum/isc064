...
namespace ISC064.ADMINJUAL
{
    public partial class Agent : System.Web.UI.Page
    {
        ...
        private void Fill(string Project)
        {
            ...
            Rpt.NoData(x, rs, "Tidak ada agent dengan nama yang berawalan : " + Request.QueryString["k"] + ".");
            ...
            for (int i = 0; i < rs.Rows.Count; i++)
            {
                ...
                //x.Append("<p><a show-modal='#ModalPopUp' modal-title='Edit Agent' modal-url='AgentEdit.aspx?NoAgent=" + rs.Rows[i]["NoAgent"] + "'>"
                x.Append("<p><a href=\"javascript:call('" + rs.Rows[i]["NoAgent"] + "')\">"
                    + rs.Rows[i]["Nama"]
                    + " (" + rs.Rows[i]["NoAgent"].ToString().PadLeft(5, '0') + ")"
                    + "</a></p>");
            }
