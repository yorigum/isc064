...
namespace ISC064.ADMINJUAL
{
    public partial class Log : System.Web.UI.Page
    {
        ...
        private void Fill2()
        {
            ...
            string popup = "";
            if (href.SelectedItem.Value == "MS_UNIT_LOG")
            {
                popup = ",'<a href=\"javascript:popEditUnit('''+Pk+''')\">' + Pk +  '</a>'";
            }
            else if (href.SelectedItem.Value == "MS_AGENT_LOG")
            {
                popup = ",'<a href=\"javascript:popEditAgent('''+Pk+''')\">' + Pk +  '</a>'";
            }
            else if (href.SelectedItem.Value == "ISC064_MARKETINGJUAL..REF_SKEMA_LOG")
            {
                popup = ",'<a href=\"javascript:popSkema('''+Pk+''')\">' + Pk +  '</a>'";
            }
            else if (href.SelectedItem.Value == "ISC064_MARKETINGJUAL..REF_AGENT_TIPE_LOG")
            {
                popup = ",'<a href=\"javascript:popEditTipeSales('''+Pk+''')\">' + Pk +  '</a>'";
            }
            else if (href.SelectedItem.Value == "ISC064_MARKETINGJUAL..REF_AGENT_LEVEL_LOG")
            {
                popup = ",'<a href=\"javascript:popEditAgentLevel('''+Pk+''')\">' + Pk +  '</a>'";
            }
            else
                popup = ",'+Pk+'";
        ...
        private void Fill()
        {
            ...
            for (int i = 0; i < rs.Rows.Count; i++)
            {
                ...
                c = new TableCell();
                c.Text = "<a show-modal='#ModalPopUp' modal-title='Log Detil' modal-url='LogDetil.aspx?LogID=" + rs.Rows[i]["LogID"] + "&tb=" + tb.SelectedValue + "&sumber=" + tb.SelectedItem.Text + "'>"
                ...
                if (href.SelectedItem.Text != "NA")
                {
                    ...
                    c.Text = "<a show-modal='#ModalPopUp' modal-title='Detail' modal-url=" + popup + ">" + rs.Rows[i]["Pk"].ToString() + "</a>";
                }
                else
