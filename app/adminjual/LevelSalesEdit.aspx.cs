...
namespace ISC064.ADMINJUAL
{
    ...
    public partial class LevelSalesEdit : System.Web.UI.Page
    {
        ...
        {
            string strSql2 = "SELECT * FROM " + Mi.DbPrefix + "MARKETINGJUAL..REF_AGENT_LEVEL WHERE Project = '" + project.SelectedValue + "' AND Tipe = " + tipe.SelectedValue ;
        ...
        private void Fill()
        {
            ...
            else
            {
                ...
                string Tipe = Db.SingleString("SELECT Tipe FROM "+Mi.DbPrefix+"MARKETINGJUAL..REF_AGENT_TIPE WHERE ID = '"+ rs.Rows[0]["Tipe"].ToString() + "'");
                tipe.Items.Add(new ListItem("Sekarang : " + Tipe, rs.Rows[0]["Tipe"].ToString()));
                string Parent = Db.SingleString("SELECT Nama FROM " + Mi.DbPrefix + "MARKETINGJUAL..REF_AGENT_LEVEL WHERE LevelID = '" + rs.Rows[0]["ParentID"].ToString() + "'");
                Cf.SelectedValue(parent, Parent);
