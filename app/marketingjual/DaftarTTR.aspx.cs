...
{
    public partial class DaftarTTR : System.Web.UI.Page
    {
        ...
        private void Fill()
        {
            ...
            string strSql = "SELECT *"
                + ",(SELECT Nama FROM " + Mi.DbPrefix + "SECURITY..USERNAME WHERE UserID = a.UserID) AS Nama"
                + ",CASE CaraBayar"
                + "		WHEN 'TN' THEN 'TUNAI'"
                ...
                + " END AS CaraBayar2"
                + " FROM MS_TTR a INNER JOIN MS_UNIT b ON a.Unit=b.NoUnit"
                + " WHERE a.NoTTR + a.NoReservasi + a.Unit + a.Customer + a.Ket + a.NoBG "
            ...
            Rpt.NoData(rpt, rs, "Tidak ditemukan data tanda terima reservasi dengan keyword diatas.");
            ...
            for (int i = 0; i < rs.Rows.Count; i++)
            {
                ...
                {
                    c.Text = "<a href=\"javascript:callSource('" + rs.Rows[i]["NoTTR"] + "', '" + Request.QueryString["status"] + "')\" " + strike + ">"
                        + rs.Rows[i]["NoTTR"] + "</a>"
                        + "<br><i>" + status + "</i>"
                        ;
                }
                ...
                {
                    c.Text = "<a href=\"javascript:call('" + rs.Rows[i]["NoTTR"] + "')\" " + strike + ">"
                        + rs.Rows[i]["NoTTR"] + "</a>"
                        + "<br><i>" + status + "</i>"
                        ;
                }
                ...
                c = new TableCell();
                c.Text = " No. " + rs.Rows[i]["NoReservasi"]
                    + "<br>" + rs.Rows[i]["Unit"]
