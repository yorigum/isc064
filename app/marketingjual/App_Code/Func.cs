...
namespace ISC064.MARKETINGJUAL
{
    public class Func
    {
        ...
        #endregion
        #region public static bool CekKontrak(string NoKontrak)
        public static bool CekKontrak(string NoCustomer)
        {
            bool isAkunting = false;
            string strSql = "SELECT NoCustomer FROM " + Mi.DbPrefix + "MARKETINGJUAL..MS_KONTRAK WHERE NoCustomer = '" + NoCustomer + "'";
        ...
        {
            bool isAkunting = false;
            int cAkunting = Db.SingleInteger("SELECT COUNT(Akunting) FROM " + Mi.DbPrefix + "FINANCEAR..MS_TTS WHERE NoTTS = " + NoTTS);

            if (cAkunting > 0)
            {
                int Akunting = Db.SingleInteger("SELECT ISNULL(Akunting,0) FROM " + Mi.DbPrefix + "FINANCEAR..MS_TTS WHERE NoTTS = " + NoTTS);
        ...
        #endregion
        #region public static bool CekTTS(string NoKontrak)
        public static bool CekTTS(string NoKontrak)
        {
            bool isAkunting = false;
            string strSql = "SELECT NoTTS FROM " + Mi.DbPrefix + "FINANCEAR..MS_TTS WHERE Ref = '" + NoKontrak + "'";
            DataTable rsTTS = Db.Rs(strSql);

            if (rsTTS.Rows.Count > 0)
            {
                isAkunting = true;
            }
        ...
        #endregion
        #region public static bool CekMemo(string NoKontrak)
        public static bool CekMemo(string NoKontrak)
        {
            bool isAkunting = false;
            string strSql = "SELECT NoMemo FROM " + Mi.DbPrefix + "FINANCEAR..MS_MEMO WHERE Ref = '" + NoKontrak + "'";
            DataTable rsTTS = Db.Rs(strSql);
        ...
        #region public static List<Tagihan> ListTagihan(int SkemaID,decimal NilaiKontrak,DateTime Tgl)
        public static List<Tagihan> ListTagihan(object SkemaID, decimal NilaiKontrak, DateTime Tgl)
        {
            ...
            foreach (DataRow r in rs.Rows)
            {
                if ((string)r["Tipe"] == "BF")
                {
                    totalBF += (string)r["TipeNominal"] == "%" ? (NilaiKontrak / 100 * Convert.ToDecimal(r["Nominal"])) : Convert.ToDecimal(r["Nominal"]);
                }
            }
            if (CountSebarBF != 0)
            ...
            {
                // set NilaiTagihan
                decimal NilaiTagihan = (string)r["TipeNominal"] == "%" ? ((NilaiKontrak - totalBF) / 100 * Convert.ToDecimal(r["Nominal"])) : Convert.ToDecimal(r["Nominal"]);
                ...
                {
                    NilaiTagihan = (NilaiKontrak - TotalWithOutPEL) / 100 * Convert.ToDecimal(r["Nominal"]);
                }
                ...
                if (index == rs.Rows.Count - 1)
                    NilaiTagihan = (NilaiKontrak - totalBF) - Temp;                
        ...
        public class Tagihan
        {
            ...
            public bool PotongBF { get; set; }
        }

        #region public static void UnitSedangClosing(string NoStock)
        public static void UnitSedangClosing(string NoStock)
        {
            bool adaYangLain = Db.SingleInteger("SELECT COUNT(*) FROM ISC064_MARKETINGJUAL..MS_UNIT_CLOSING"
                + " WHERE NoStock = '" + NoStock + "'"
                + " AND UserID <> '" + ISC064.Act.UserID + "'") > 0;

            if (adaYangLain)
                HttpContext.Current.Response.Redirect("/CustomError/ClosingDouble.html");
        }
        #endregion
    }
}
