...
{
	public partial class PrintKwitansiGabung : System.Web.UI.Page
	{
		...
		{
			PrintKwitansiGabungTemplate uc = (PrintKwitansiGabungTemplate) Page.LoadControl("PrintKwitansiGabungTemplate.ascx"); 
		...
		private void Fill()
		{
			...
			string strSql = "SELECT PrintKwitansiGabung FROM MS_KONTRAK WHERE NoKontrak = '" + NoKontrak + "'";
			DataTable rs = Db.Rs(strSql);
			if(rs.Rows.Count==0)
				Response.Redirect("/CustomError/NoPrint.html");
			else
			{
				count.Text = rs.Rows[0]["PrintKwitansiGabung"].ToString();
				if((int)rs.Rows[0]["PrintKwitansiGabung"]==0)
		...
		private void Tampil()
		{
			...
			//increment
			Db.Execute("UPDATE MS_KONTRAK SET PrintKwitansiGabung = PrintKwitansiGabung + 1 WHERE NoKontrak = '" + NoKontrak + "'");
			...
			DataTable rs = Db.Rs("SELECT "
				...
				+ ",PrintKwitansiGabung AS [Print Counter]"
