...
{
	public partial class LaporanPPNUangMuka : System.Web.UI.Page
	{
		...
		private void Fill()
		{
			...
			for(int i = 0; i < rs.Rows.Count; i++)
			{
				...
				//if(JenisPPN.SelectedItem.Text == "PEMERINTAH")
				if(JenisPPN.SelectedItem.Text == "KONSUMEN")
					Netto = Convert.ToDecimal(rs.Rows[i]["NilaiPelunasan"]) / (decimal)1.1;
				else
					Netto = Convert.ToDecimal(rs.Rows[i]["NilaiPelunasan"]);
				...
				c = new TableCell();
				PPN = Convert.ToDecimal(rs.Rows[i]["NilaiPelunasan"]) - Netto;
