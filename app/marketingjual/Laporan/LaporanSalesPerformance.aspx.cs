...
namespace ISC064.MARKETINGJUAL.Laporan
{
    ...
    public partial class LaporanSalesPerformance : System.Web.UI.Page
    {
        ...
        protected void pdf_Click(object sender, System.EventArgs e)
        {
            ...
            string Nama = "Laporan Sales Performance";
            string Link = "";
            ...
            string nfilename = "SalesPerformance" + NoAttachment + ".pdf";
            ...
            string save = Mi.PathFilePDFReport + "SalesPerformance" + rs.Rows[0]["AttachmentID"] + ".pdf";

            //declare parameter
            string Agent = ddlAgent.SelectedValue;
            string FilterTgl = filtertgl.SelectedValue;
            ...
            string link = Mi.PathAlamatWeb + "marketingjual/LaporanPDF/PDFLaporanSalesPerformance.aspx?id=" + rs.Rows[0]["AttachmentID"] + "&filtertgl=" + FilterTgl + "&agent=" + Agent + "&userid=" + UserID + "&statussales=" + status.SelectedValue + "&project=" + Project;

            //update link
            Db.Execute("UPDATE ISC064_MARKETINGJUAL..LapPDF SET Link= '" + link + "' WHERE AttachmentID = " + NoAttachment);

            //format page
            p.StartInfo.Arguments = "--orientation landscape --page-width 35cm --page-height 85cm --margin-left 0 --margin-right 0 --margin-top 0.25cm --margin-bottom 0 " + link + " " + save;
        ...
        private void init()
        {
            ...
            sampai.Text = Cf.Day(DateTime.Now);
            tglasof.Text = Cf.Day(DateTime.Now);

            string Agent = "";
            if (ddlAgent.SelectedIndex != 0)
                Agent += " AND NoAgent = " + ddlAgent.SelectedValue;
            ...
            if (project.SelectedIndex > 0) Project = "AND Project = '" + project.SelectedValue + "'";

            string strSql = "SELECT NoAgent, Nama FROM MS_AGENT WHERE Status = 'A'"
                            + Project
                            + Agent
                            + aa
                            + "ORDER BY Nama";
        ...
        private bool valid()
        {
            ...
            if (filtertgl.SelectedValue.ToString() == "0")
            {
                ...
                if (Cf.isTgl(dari) && Cf.isTgl(sampai))
                {
                    ...
                    {
                        x = false;
                        periodec.Text = "Hanya bisa menampilkan laporan dalam periode tahun yang sama";
                    }
                    else
                    {
                        periodec.Text = "";
        ...
        private void Fill()
        {
            ...
            DateTime Dari = Convert.ToDateTime(dari.Text);
            DateTime Sampai = Convert.ToDateTime(sampai.Text);
            DateTime AsOf = Convert.ToDateTime(tglasof.Text);
            ...
            else if (status.SelectedValue == "I")
            {
                if (Agent == "")
                {
                    Agent += " AND Status='I'";
            ...
            if (project.SelectedIndex > 0) Project = "WHERE Project = '" + project.SelectedValue + "'";

            //Response.Write(ddlAgent.SelectedValue);
        ...
        #endregion
        protected void filtertgl_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (filtertgl.SelectedValue.ToString() == "0")
            {
                range.Visible = true;
