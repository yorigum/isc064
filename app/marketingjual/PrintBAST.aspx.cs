...
{
	public partial class PrintBAST : System.Web.UI.Page
	{
		...
		private void Fill()
		{
			...
			string strSql = "SELECT PrintBAST FROM MS_KONTRAK WHERE NoKontrak = '" + NoKontrak + "' AND ST = 'D'";
			DataTable rs = Db.Rs(strSql);
			if(rs.Rows.Count==0)
				Response.Redirect("/CustomError/NoPrint.html");
			else
			{
				count.Text = rs.Rows[0]["PrintBAST"].ToString();
                if ((int)rs.Rows[0]["PrintBAST"] == 0)
                {
                    Tampil(); //langsung tampil
                    ConvertPdf();
                    Response.Redirect(Param.PathLinkFilePDFMarketingJual + NoKontrak.Replace("/", "_").Replace("\\", "_") + Project + "_BAST.pdf");
                }
				else
				{
					//mekanisme reprint
		...
		private void Tampil()
		{
			...
			//increment
			Db.Execute("UPDATE MS_KONTRAK SET PrintBAST = PrintBAST + 1 WHERE NoKontrak = '" + NoKontrak + "'");
        ...
        private void ConvertPdf()
        {
            ...
            string myHtml = "http://localhost:8034/marketingjual/PrintBAST1.aspx?NoKontrak=" + NoKontrak + "&project=" + Project;

            string save = Param.PathFilePDFMarketingJual + NoKontrak.Replace("/", "_").Replace("\\", "_") + Project + "_BAST.pdf";
        ...
        private void Valid(string Username)
		{
            ...
            Tampil();
            string file = Param.PathFilePDFMarketingJual + NoKontrak.Replace("/", "_").Replace("\\", "_") + Project + "_BAST.pdf";
